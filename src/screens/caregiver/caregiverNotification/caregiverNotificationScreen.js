...
import caregiverNotificationStyle from "./caregiverNotificationStyle"
...
class caregiverNotificationScreen extends React.Component {

    constructor(props) {
        super(props);
        AppUtils.analyticsTracker("Caregiver Notifications");
        this.state = ({
            notifications: '',
            isLoading: false,
            isDataVisible: false,
    ...
    async getNotifications(isRefresh) {
        ...
        try {
            ...
            let response = await SHApiConnector.getCaregiverNotifications(page.page);
            ...
            AppUtils.console("CAAREGIVER_NOTIFICATION", e)
        }
    }
        ...
        return (
            <ElevatedView elevation={10}>
                {(this.state.isFooterLoading) ?
                    <View style={caregiverNotificationStyle.renderFooter} onPress={() => this.getNotifications(false)}>
                        <TouchableHighlight style={{backgroundColor: AppColors.colorPrimary, width: width}}
                            ...
                            <Text style={caregiverNotificationStyle.footerText}>{strings('string.label.see_more')}</Text>
                        </TouchableHighlight>
                    </View>
                    :
                    <View/>}
    ...
    render() {
        return (
            <View style={caregiverNotificationStyle.main}>
                    ...
                    <View>
                        {(!this.state.noNotifications) ?
                            <View style={{
                                ...
                                />
                            </View> : <View style={caregiverNotificationStyle.cancelView}>
                                ...
                                }}>{strings('string.label.no_notification')}</Text>
                            </View>}
                    </View> : <View/>}
                <ProgressLoader
        ...
        return (
            <ScrollView showsVerticalScrollIndicator={false}>
                {item.item.isRead == 'NO' || item.item.isRead == 'No' || item.item.isRead == 'no' ?
                    ...
                    <TouchableHighlight onPress={() => Actions.caregiverTab({isCaregiverBookingUpdated: true})}
                                        underlayColor='transparent'
                                        style={{marginBottom: verticalScale(20)}}>
                        <View style={caregiverNotificationStyle.notificationView}>
                            <CachedImage
                                source={{uri: AppUtils.handleNullClinicImg(item.item.image)}}
                                style={caregiverNotificationStyle.notificationImage}
                            />
