 ...
 * Mostly this will be helpful in setting date in DatePickerIOS (iOS) of React-Native.
 *
 * This library takes help from great date and timezone management libraries, "Moment" and "Moment-Timezone".
 * Tested only in iOS Simulator so far (react-native). Tested by changing the system timezone settings.
 *
 */
let moment = require('moment');
let momentTZ = require("moment-timezone");
let TAG = "RN_TIMEZONE_OFFSET"
let offsetMap = [{
...
timezoneOffset.prototype.toLocalTZTypical = function (serverDate) {
    try {
        var myTZ = (momentTZ.tz.guess());
        var mmTZ = momentTZ(serverDate);
        var txx = mmTZ.tz(myTZ).format('YYYY-MM-DD HH:mm');
        var txxx = new Date(txx);
        return txxx;
    } catch (err) {
        return false;
    }
...
timezoneOffset.prototype.toLocalTZ = function (serverDate) {
    let obj = {};
    let myTZ = momentTZ.tz.guess();
    let isTZFound = false;
    for (var i = 0; i < offsetMap.length; i++) {
        var eachZone = offsetMap[i];
        var utcArr = eachZone.utc;
        for (j = 0; j < utcArr.length; j++) {
            if (utcArr[j] == myTZ) {
                obj = eachZone;
                isTZFound = true;
                break;
    ...
    if (isTZFound) {
        var ex = moment(serverDate).toDate();
        var offset = -1 * (obj.offset);
        var offsetCalc = (offset + (obj.isdst ? 1 : 0)) * 60;
        ex.setMinutes(ex.getMinutes() + ex.getTimezoneOffset() + offsetCalc);
    } else {
        ex = this.toLocalTZTypical(serverDate);
    }
    return ex;
};

timezoneOffset.prototype.getMyTimezone = function (serverDate) {
    let myTZ = (momentTZ.tz.guess());
    return myTZ;
};

module.exports = new timezoneOffset();